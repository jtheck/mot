...
(function(win){
  var GETFIRE = win.GETFIRE = function(config) {
    ...
    var defaultName = config.defaultName || "Guest";
    var startOpen = config.startOpen || false;
    var fullHeight = config.fullHeight || false;
    var clickAwayIgnore = config.clickAwayIgnore || false;
    var clickBubbleClose = config.clickBubbleClose || false;
    ...
    var gCard = {};//Name, gCardID, gCardColor;
    var aCard = {id:makeID(), name:defaultName, color:makeColor()};
    var tCard = {};
    GETFIRE.preview = [];
    ...
    var $respondB = newDiv({id:"gf_mRespond", className: "gf_hcmod", content: respondSVG, title:"reply to this message"});
    ...
    GETFIRE.$preview = newDiv({id:"gf_preview", content:'123'});
    ...
    // };
    // preview name
    $pName = newDiv({id:"gf_pname", content: "Connecting..."});
    ...
    // avatar popup
    var $popup = newDiv({id:"gf_avatar_popup", title:"visit ecard"});
    ...
    // head
    var $head = newDiv({id:"gf_topic_head", content:"Connecting..."});
    ...
    // pick a card button
    var $pac = newDiv({id:"gf_pac", content:"Pick a Card"});
    $settings.append($pac);

    // pick a card dropdown
    var $pacUI = newDiv({id:"gf_pac_ui", content:""});
    $pac.append($pacUI);

    // pick a card anon wrapper
    var $pacUIAnon = newDiv({id:"gf_pac_ui_anon", content:""});
    $pacUI.append($pacUIAnon);


    // new color button
    var $getColor = newDiv({id:"gf_get_color", title:"new color"});
    $getColor.style.borderColor = aCard.color;
    $pacUIAnon.append($getColor);
    $getColor.onclick = function() {
      aCard.color = $getColor.style.borderColor = makeColor();
    };

    // new name textbox
    var $getName = document.createElement("input");
    $getName.id = "gf_get_name";
    $getName.type = "text";
    $getName.title = "custom name";
    $getName.value = aCard.name;
    $pacUIAnon.append($getName);
    $getName.onkeyup = function() {
      aCard.name = $getName.value.substr(0,20);
    };

    // pick a card user cards
    var $pacUICard = newDiv({id:"gf_pac_card", content:""});
    $pacUI.append($pacUICard);

    $pacUICard.onclick = function(){
      setCard(gCard);
    }
    $pacUIAnon.onclick = function(){
      setCard(aCard);
    }

    // card
    var $card = newDiv({id:"gf_card", title:"your card"});
    GETFIRE.$topic.append($card);
    $card.style.borderColor = aCard.color;
    $card.onclick = function() {
      $settings.style.display = $settings.style.display === "block" ? "none" : "block";
    };

    // card name
    var $cardName = newDiv({id:"gf_card_name", content:aCard.name});
    $card.append($cardName);

    // toggle size button
    var $tsb = newDiv({id:"gf_tsb", content:" ➕ ", title:"toggle size"});
    ...
    // pick a card guide button
    var $pacGuide = newDiv({id:"gf_pacGuide", content:"<span id='gf_pg_b'>pick a card</span> and start chatting!"});
    GETFIRE.$messages.append($pacGuide);
    var $pgb = document.getElementById("gf_pg_b");
    $pgb.addEventListener('mouseover', function(e){
      $card.classList.add("gf_card_mod");
    });
    $pgb.addEventListener('mouseleave', function(e){
      $card.classList.remove("gf_card_mod");
    });
    $pgb.addEventListener('click', function(e){
      $card.click();
    });

    // last activity
    var $lastActivity = newDiv({id:"gf_lastAcivity"});
    GETFIRE.$messages.append($lastActivity);

    // message controls
    var $mControls = newDiv({id:"gf_mControls", className:"gf_hcmod"});
    document.body.append($mControls);
    $mControls.append($respondB);
    var $mTime = newDiv({id:"gf_mTime", className:"timeago gf_hcmod", content:"eons hence", title:"time since this message was sent"});
    ...
    // $form.setAttribute("name", "gf_pubm");
    $form.setAttribute("id", "gf_pubm");
    // text area
    ...
    $submit.setAttribute('title',"send message");
    $submit.id = "gf_submit";
    ...
    GETFIRE.pn.addListener({
      ...
      message: function(m) {
        // console.log(m);
        if (m.message.type == "new_message") renderMessage(m.message.content);
      }
    });
    ...
    function subscribe() {
      ...
      xhr.withCredentials = true; // only verified sites
      ...
      xhr.onload = function(){
        ...
        if (xhr.status === 200) {
          ...
          if (data) {
            ...
            // intent
            $intent = newDiv({id:"gf_ideal", content:data.summary || "No topic intent."});
            ...
            var user = JSON.parse(xhr.responseText).card;
            var sub = JSON.parse(xhr.responseText).sub;
            if (user) { // get selected/reserve card
              gCard.id = user.hashish;
              gCard.name = user.name;
              gCard.color = user.color;
              // console.log(user);
              $pacUICard.innerHTML = gCard.name;
              $pacUICard.style.borderColor = gCard.color;
              var iurl = "url(https://s3.amazonaws.com/getfire-paperclip-"+env+"/avatars/"+gCard.id+"/half.jpg)";
              $pacUICard.style.backgroundImage = iurl;
              $pacUICard.style.display = "block";
            } else {
              $pacUICard.style.display = "none";

              // log in
              var urlName = topicName.replace(/ /g,"_");
              $gflink = newDiv({id:"gf_link", content:"Sign in for more: <br> <a href='https://getfire.net/"+urlName+"' target='_blank'>GetFire.net/"+urlName+"</a>"});
              $settings.append($gflink);
            }
            sub ? setCard(gCard) : setCard(aCard);

            // last activity indicator
            if (messages[0]) {
              $lastActivity.innerHTML = "(last activity <span class='timeago' id='lata' datetime='"+messages[Math.max(0, messages.length - 1)].created_at+"'></span>)";
              var $lata = document.getElementById("lata");
              ta.render($lata);
            } else {
              $lastActivity.innerHTML = "(no recent messages)";
            }
    ...
    GETFIRE.postMessage = function(msg) {
      ...
      xhr.onload = function(){
        ...
        if (xhr.status === 200) {
          ...
          document.getElementById("gf_submit").value = "Send";
          document.getElementById("gf_message_entry").value = "";
        } else {
          // failure
          document.getElementById("gf_submit").disabled = false;
    ...
    function renderMessage(m) {
      ...
      GETFIRE.$messages.insertBefore($tMsg, $lastActivity);
    ...
    function setCard(card) {
      ...
      $card.style.borderColor = card.color;
      var iurl = "url(https://s3.amazonaws.com/getfire-paperclip-dev/default-avatars/ruination-one/"
      +Math.floor(Math.random()*36)+"/half.jpg)";
      $card.style.backgroundImage = iurl;
    }
    ...
    function gfClick(e) {
      ...
      if (path.includes($wrapper) || path.includes($popup)) {
        // close settings
        if (!path.includes($settings) && !path.includes($card) && !path.includes($pgb)) $settings.style.display = "none";

        // open or close popup
        var foundPopup = false;
        for (var i=0; i< path.length; i++) {
          // if (!path[i].classList.value) continue;
    if ( typeof(path[i].classList) === "undefined" || path[i].classList === null ) continue;
    if ( typeof(path[i].classList.value) === "undefined" || path[i].classList.value === null ) continue;
          if (path[i].classList && path[i].classList.value.includes("gf_mName")) foundPopup = path[i];
        }
        if (foundPopup == false) {
